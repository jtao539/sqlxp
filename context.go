...
// SelectE 多表查询 列表查找, 重要 : 不能匹配具体的字段, dest为要查找的数据类型数组, request为查询条件结构体(NULL类型), table 为表名称, otherFiledMap为表字段与sql映射,  factors 为条件和参数的map
func (s *SqlxP) SelectE(dest interface{}, request interface{}, table string) (err error, total int) {
	str, params, countStr := SafeSelectE(request, table)
	err = s.DB.Select(dest, str, params...)
	s.DB.Get(&total, countStr, params...)
	return
}

// SelectEPQ SelectE PQ版本
func (s *SqlxP) SelectEPQ(dest interface{}, request interface{}, table string) (err error, total int) {
	str, params, countStr := SafeSelectPQ(request, table)
	err = s.DB.Select(dest, str, params...)
	s.DB.Get(&total, countStr, params...)
	return
}
